<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:71:"F:\xampp\htdocs\chzu\public/../application/xt\view\html\admin_list.html";i:1566889013;}*/ ?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
//      表格渲染函数
            var demoIns = table.render({
                ...
                , method: 'Get'
                , height: 'full-200'
                , url: '/chzu/public/xt/users?name=' + name
                , page: true
            ...
            table.on('tool(user)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'del') { //删除
                    layer.confirm('确定要删除吗？', function (index) {
                        ...
                        //向服务端发送删除指令
                        $.ajax({
                            url: "/chzu/public/xt/users/" + data['key_id'],
                            data: {
                                '_method': 'delete'
                            },
                            type: "post",
                ...
                if (data.length > 0) {
                    layer.confirm('确定要删除这' + data.length + '条数据吗？', function () {
                        for (var i = 0; i < data.length; i++) {
                            ...
                            function ddd() {
                                $.ajax({
                                    url: "/chzu/public/xt/users/" + employeesId,
                                    data: {
                                        '_method': 'delete'
                                    },
                                    type: "post",
